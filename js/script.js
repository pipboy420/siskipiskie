  ...
  if (siteOverlay) {
    siteOverlay.addEventListener('click', () => {
      ...
      if (qbModal && qbModal.getAttribute('aria-hidden') === 'false') {
        // Используем универсальную функцию закрытия модалки
        closeModal();
      }
...
const qbModal = document.getElementById('quickBookingModal');
if (qbModal) {
  qbModal.addEventListener('click', function(e) {
    // Если клик произошёл вне модального контента, закрыть модальное окно
...
function renderReviews() {
  ...
  reviewsContainer.style.margin = '0 auto';

  reviewsContainer.innerHTML = "";
  siteContent.reviews.forEach(rev => {
    const card = document.createElement('div');
    card.className = 'review__card';
    // Устанавливаем фиксированную ширину для карточки
